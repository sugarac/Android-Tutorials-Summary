...
3. RecyclerView: [https://github.com/codepath/android\_guides/wiki/Using-the-RecyclerView](https://github.com/codepath/android_guides/wiki/Using-the-RecyclerView)
   ...
   > 1. Add RecyclerView dependency into gradle file \(how?\)
   > 2. Replace the ListView in activity\_main.xml with RecyclerView
   > 3. Create view holder class 可以理解为坑，即初始化（某个功能）用到的各种View（将layout文件中的View赋值，用ButterKnife）
   > 4. Create adapter class
   > 5. Choose layout manager and use RecyclerView in MainActivity 组合
